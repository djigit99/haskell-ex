...
main = do
  c <- connectPostgreSQL "host=localhost dbname=postgres user=mytimetable password=root"
  ...
  print allAuthors
  newAuthorId <- createAuthor "Kate" "Mazur" newSourceId c
  print newAuthorId
  newAuthor <- readAuthor c newAuthorId
  print newAuthor
  updateAuthor newAuthorId "Kate1" "Mazur1" newSourceId c
  ...
  print emptyAuthor
  newAuthorId <- createAuthor "Kate1" "Mazur1" newSourceId c
  putStrLn "Author end"
